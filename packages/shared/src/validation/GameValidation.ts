 ...
 * Waliduje rozmiar
 */
export function validateSize(size: any): ValidationResult {
  const errors: string[] = [];
 ...
 * Waliduje Game ID
 */
export function validateGameId(id: any): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validatePlayerName(name: any): ValidationResult {
  const errors: string[] = [];
 ...
 * Waliduje zasoby
 */
export function validateResources(resources: any): ValidationResult {
  const errors: string[] = [];
  ...
  for (const [type, amount] of Object.entries(resources)) {
    ...
    if (typeof amount !== 'number' || isNaN(amount as number) || (amount as number) < 0) {
      errors.push(`Invalid resource amount for ${type}: must be a non-negative number`);
    }

    if ((amount as number) > 1000000) {
 ...
 * Waliduje jednostkę
 */
export function validateUnit(unit: any): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validateUnitStats(stats: any): ValidationResult {
  const errors: string[] = [];
 ...
 * Waliduje budynek
 */
export function validateBuilding(building: any): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function validatePlayerAction(action: any): ValidationResult {
  const errors: string[] = [];
 ...
 */
export function checkRateLimit(playerId: GameId, action: string, rateLimits: Map<string, number>): boolean {
