...
const Renderable_1 = require("../components/Renderable");
const IsometricUtils_1 = require("../../math/IsometricUtils");
const Vector2_1 = require("../../math/Vector2");
/**
 * System renderowania dla ECS
 */
class RenderingSystem extends System_1.RenderSystem {
    constructor(world, renderingEngine) {
        super(world);
    ...
    getRequiredComponents() {
        return [Transform_1.Transform, Renderable_1.Renderable];
    }
    render(deltaTime) {
        if (!this.enabled)
            return;
        ...
        const sortedEntities = this.entities.slice().sort((a, b) => {
            const renderableA = a.getComponent(Renderable_1.Renderable);
            const renderableB = b.getComponent(Renderable_1.Renderable);
        ...
        for (const entity of sortedEntities) {
            ...
            const renderable = entity.getComponent(Renderable_1.Renderable);
            ...
            const worldPos = Vector2_1.Vector2.add(transform.position, renderable.offset);
    ...
    renderEntity(entity, transform, renderable, ctx, deltaTime) {
        ctx.save();
        // Pozycja renderowania
        const renderPos = Vector2_1.Vector2.add(transform.position, renderable.offset);
        const camera = this.renderingEngine.getCamera();
        ...
        switch (renderable.renderType) {
            case Renderable_1.RenderType.SHAPE:
                this.renderShape(renderable, ctx);
                break;
            case Renderable_1.RenderType.SPRITE:
                this.renderSprite(renderable, ctx);
                break;
            case Renderable_1.RenderType.TEXT:
                this.renderText(renderable, ctx);
                break;
            case Renderable_1.RenderType.ISOMETRIC_TILE:
                this.renderIsometricTile(renderable, ctx, transform);
                break;
            case Renderable_1.RenderType.CUSTOM:
                this.renderCustom(renderable, ctx, renderPos, transform.scale, transform.rotation);
                break;
        }
        ctx.restore();
    }
    renderShape(renderable, ctx) {
        if (!renderable.shapeData)
            return;
        ...
        switch (shape.type) {
            case Renderable_1.ShapeType.RECTANGLE:
                ...
                break;
            case Renderable_1.ShapeType.CIRCLE:
                if (shape.radius) {
                    ctx.beginPath();
                ...
                break;
            case Renderable_1.ShapeType.LINE:
                ...
                break;
            case Renderable_1.ShapeType.POLYGON:
    ...
    renderSprite(renderable, ctx) {
        if (!renderable.spriteData)
            return;
        ...
        if (sprite.sourceRect) {
            ...
            ctx.drawImage(image, src.x, src.y, src.width, src.height, -destWidth / 2, -destHeight / 2, destWidth, destHeight);
        }
        else {
            ctx.drawImage(image, -image.width / 2, -image.height / 2);
        }
    }
    renderText(renderable, ctx) {
        if (!renderable.textData)
            return;
    ...
    renderIsometricTile(renderable, ctx, transform) {
        if (!renderable.isometricGridPos)
            return;
        // Reset transformacji dla kafelka izometrycznego
        ctx.restore();
        ctx.save();
        const gridPos = renderable.isometricGridPos;
        const screenPos = IsometricUtils_1.IsometricUtils.gridToScreenCenter(gridPos.x, gridPos.y);
        const camera = this.renderingEngine.getCamera();
        const finalScreenPos = camera.worldToScreen(screenPos);
        const halfWidth = IsometricUtils_1.IsometricUtils.HALF_TILE_WIDTH * camera.zoom;
        const halfHeight = IsometricUtils_1.IsometricUtils.HALF_TILE_HEIGHT * camera.zoom;
        ctx.fillStyle = renderable.color;
        ctx.globalAlpha = renderable.opacity;
        ctx.beginPath();
    ...
    renderCustom(renderable, ctx, position, scale, rotation) {
        if (!renderable.customRenderFunction)
            return;
