...
export class ValidationMiddleware {
  ...
  static validateIncomingMessage(socket: Socket, messageType: string, data: any): boolean {
    try {
      ...
      if (!Object.values(ClientMessageType).includes(messageType as ClientMessageType)) {
   ...
   */
  private static validateMessageByType(socket: Socket, messageType: string, data: any): boolean {
    switch (messageType) {
   ...
   */
  private static validateJoinLobby(socket: Socket, data: any): boolean {
   ...
   */
  private static validateCreateRoom(socket: Socket, data: any): boolean {
   ...
   */
  private static validateJoinRoom(socket: Socket, data: any): boolean {
   ...
   */
  private static validatePlayerAction(socket: Socket, data: any): boolean {
   ...
   */
  private static validateChatMessage(socket: Socket, data: any): boolean {
   ...
   */
  private static sanitizeIncomingData(data: any): void {
   ...
   */
  private static sanitizeOutgoingData(data: any): void {
    if (typeof data !== 'object' || data === null) return;
   ...
   */
  private static sendValidationError(socket: Socket, errorCode: string, message: string): void {
    socket.emit('error', {
  ...
  static canSocketSendMessage(socket: Socket, messageType: string): boolean {
    ...
    const now = Date.now();
    const lastMessage = (socket as any).lastMessageTime?.[messageType] || 0;

    if (now - lastMessage < limit) {
      ValidationMiddleware.sendValidationError(
        socket, 
        'RATE_LIMIT', 
    ...
    if (!(socket as any).lastMessageTime) {
      (socket as any).lastMessageTime = {};
    }
    (socket as any).lastMessageTime[messageType] = now;
