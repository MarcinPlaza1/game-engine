...
import { createAIPlayer, AIDifficulty } from './AIPlayer.js';
...
export class GameManager extends EventEmitter {
  private games: Map<GameId, GameInstance> = new Map();
  private gameRooms: Map<GameId, GameRoom> = new Map();
   ...
   */
  addAIPlayer(roomId: GameId, aiName?: string, difficulty: AIDifficulty = AIDifficulty.MEDIUM): boolean {
    const room = this.gameRooms.get(roomId);
    if (!room) {
