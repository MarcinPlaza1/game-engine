...
import { GameStatus, PlayerActionType, ResourceType, UnitType, BuildingType, generateGameId, DEFAULT_UNIT_COSTS, DEFAULT_BUILDING_COSTS, RESOURCE_LIMITS, GameEventType } from '@rts-engine/shared';
import { logger } from '../utils/Logger';
/**
 * Instancja pojedynczej gry
 */
export class GameInstance extends EventEmitter {
    constructor(gameId, players, gameSettings) {
        super();
     ...
     */
    createInitialGameState(gameId, players, gameSettings) {
        return {
            id: gameId,
            name: `Game ${gameId}`,
     ...
     */
    handleMoveUnits(playerId, action) {
        if (!action.targetPosition || !action.unitIds)
            return;
     ...
     */
    handleAttackTarget(playerId, action) {
        if (!action.targetId || !action.unitIds)
            return;
        action.unitIds.forEach(unitId => {
            const unit = this.gameState.units.find(u => u.id === unitId && u.playerId === playerId);
            if (unit) {
                unit.targetId = action.targetId;
                unit.currentAction = 'attack';
            }
        });
    }
    /**
     ...
     */
    handleTrainUnit(playerId, action) {
        const unitType = action.data?.unitType;
        const buildingId = action.data?.buildingId;
        if (!unitType || !buildingId)
            return;
        ...
        if (!this.hasEnoughResources(player, cost))
            return;
        // Odejmij zasoby
     ...
     */
    handleBuildStructure(playerId, action) {
        const buildingType = action.data?.buildingType;
        if (!buildingType || !action.targetPosition)
            return;
        const player = this.gameState.players.find(p => p.id === playerId);
        if (!player)
            return;
     ...
     */
    handleGatherResource(playerId, action) {
        if (!action.unitIds || !action.targetId)
            return;
    ...
    handleUnitDeath(unit) {
        if ('type' in unit && Object.values(UnitType).includes(unit.type)) {
            // To jest jednostka
     ...
     */
    hasEnoughResources(player, cost) {
     ...
     */
    subtractPlayerResources(player, cost) {
        for (const [resourceType, amount] of Object.entries(cost)) {
            const currentAmount = player.resources[resourceType] || 0;
            player.resources[resourceType] = Math.max(0, currentAmount - amount);
        }
    }
    /**
     * Emituje aktualizację stanu gry
     */
    emitGameUpdate(deltaTime) {
